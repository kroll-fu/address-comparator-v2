...
1. **Floor checks never drop a record that today would land in the top 3.** Floor check A uses `cityScore = 1.0` as the upper bound, which is the maximum possible — so any record dropped here has `addressScore < floor`, meaning it can't displace the current top 3. Floor check B is the exact `addressScore` — ditto. `floor = top3.minScore()` is also monotonically non-decreasing across the scan (insertions only raise the minimum, never lower it), so a record dropped against an earlier floor would also be dropped against any later floor — it cannot be one of the eventual top 3.
...
**Practical impact.** The divergence requires multiple ES records to land at the EXACT SAME float-valued addressScore as a cross-state ES record. Address scores are linear combinations of Jaro-Winkler values plus 0/0.15 (state) plus 0/0.10 (zip). Exact-tied scores between cross-state and in-state records require either (a) duplicate streets/cities across states with specific zip/state coincidences, or (b) randomly coincident JW float values. Real solar lead/customer data effectively never produces these ties in practice.

**Classification impact (narrow).** The `NoMatch` vs (`FullMatch` | `HouseholdMatch`) boundary is **unaffected** in all cases — all tied entries share the same `addressScore` and therefore fall on the same side of `addressThreshold`. The `FullMatch` vs `HouseholdMatch` distinction CAN differ between the two algorithms when tied entries have differing `nameScore` or `emailScore` values that straddle their respective thresholds (default `nameThreshold = 0.82`, `emailThreshold = 1.0`). Similarly, downstream consumers that read `topMatches[0]` only (the CSV exporter, the results-table classifier) will see the in-state record's name/email/installer/customerId/submittedDate fields rather than the cross-state record's. In practice, both effects require the same essentially-never-occurs prerequisite (exact-tied addressScore across the phase boundary).

**Phase-boundary terminology.** "In-state vs cross-state" is the common case. When `lrRecord.state === ''`, phase 1 is the `''` bucket (empty-state ES records) and phase 2 is everything else; the same divergence pattern applies (ties spanning the phase boundary prefer phase 1 records).
